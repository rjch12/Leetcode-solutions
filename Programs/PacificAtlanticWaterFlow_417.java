...
class PacificAtlanticWaterFlow_417 {
  ...
  public List<List<Integer>> pacificAtlantic(int[][] heights) {
    ...
    // filling in col wise - Top row touches pacific so call begins with (0, i), int min and pacific
    // filling in col wise - Bottom row touches atlantic so call begins with (r-1, i), int min and atlantic
    for (int i = 0; i < c; i++) {
      dfs(0, i, heights, pac, Integer.MIN_VALUE);
      dfs(r - 1, i, heights, atl, Integer.MIN_VALUE);
    }

    // filling in row wise - Left col touches pacific so call begins with (i, 0), int min and pacific
    // filling in row wise - Right col touches atlantic so call begins with (i, c-1), int min and atlantic
    for (int i = 0; i < r; i++) {
