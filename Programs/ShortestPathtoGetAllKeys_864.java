...
class ShortestPathtoGetAllKeys_864 {
    ...
    public int dfs(char[][] grid, int i, int j, Set<Character> keys, int noOfKeys, int[][] memo, int steps, boolean[][] visited) {
        if(i < 0 || i >= grid.length || j < 0 || j >= grid[0].length || grid[i][j] == '#' || noOfKeys == 0 || visited[i][j])
